...
if (title=="Senate Attendance" || title=="Senate Loyalty") {
...
if (stats.n_ind!=0) {
function votedParty1(val2) {
	let prom =totalParty(members,"party",val2)
	return (prom.reduce((sum,member)=>sum + member.votes_with_party_pct,0))/prom.length
	}
	stats.vot_dem = votedParty1("D")
	stats.vot_rep = votedParty1("R")
	stats.vot_ind = votedParty1("I")
	stats.vot_tot = (stats.vot_dem + stats.vot_rep + stats.vot_ind) / 3
}
else{
	function votedParty2(val2) {
	let prom =totalParty(members,"party",val2)
	return (prom.reduce((sum,member)=>sum + member.votes_with_party_pct,0))/prom.length
	}
	stats.vot_dem = votedParty2("D")
	stats.vot_rep = votedParty2("R")
	stats.vot_ind = 0
